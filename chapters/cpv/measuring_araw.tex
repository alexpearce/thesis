...
The uncertainty on the bin contents $\unc{b_{i}}$ is given by the effective 
number of entries, as in \cref{eqn:cpv:kinematic_weighting:neff}
\begin{equation}
  \unc{b_{i}}^{2} = \neff = \frac{%
    {\left(\sum_{i}^{N}{b_{i}}\right)}^{2}
  }{%
    \sum_{i}^{N}{b_{i}^{2}}
  }.
\end{equation}
For \ppipi\ the kinematic weights are those described in 
\cref{chap:cpv:kinematic_weighting}, where for \pKK\ there are unity.

To try to reduce the effects of experimenter bias, the fitted central values of 
\ARaw\ are blinded, and cannot be inspected until they are unblinded.
