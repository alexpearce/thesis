...
\ccbar\ cross-section, reconstructing the decay chain \LbToLcmuX\ allows for 
...
The selected final state particles are combined as \LcTophh\ candidates, which 
are then combined with muon candidates to form candidate \LbToLcmuX\ decays.
...
\PLambdab\ which will have a high \pT\ and \ac{IP}.
The muon candidate is required to have fired the \lzero\ and \hltone\ single 
muon lines, and at \hlttwo the tracks comprising the \PLambdab\ candidate are 
required to have fired the topological semileptonic \PB lines, as described in 
\cref{chap:intro:lhcb:trigger}.
...
equal to the range used in the mass fits (first described in 
...
\begin{table}
  ...
  % \begin{adjustbox}{center}
    \input{tables/cpv/selection/LcTopKK_multiple_candidates}
  % \end{adjustbox}
\end{table}
...
\begin{table}
  ...
  % \begin{adjustbox}{center}
    \input{tables/cpv/selection/LcToppipi_multiple_candidates}
  % \end{adjustbox}
\end{table}
