...
In high-energy interactions, it is the collection of these \emph{partons} that 
interact, the quarks and gluons, rather than the composite hadrons.
Parton behaviour then must be understood in order to predict production rates 
at hadron colliders.
...
shall then give an example of how the inputs are used outside of collider 
physics.
...
As they cannot be computed first principles, they must be derived be alternate 
means, and in practice are constrained using measurements of cross-sections by 
experiments.
Additional measurements of cross-sections can constrain the \acp{QCDPDF} 
further, improving their precision and hence the precision on the 
cross-sections of interest, and so there is always motivation for making such 
measurements, until the theoretical understanding improves sufficiently.
...
between predictions, indicating which ones describe the true \acp{QCDPDF}, and 
hence which treatments are most appropriate.
...
The photo-detectors are spatially arranged as an array, which is used to 
...
the double differential cross-sections, as defined by 
...
which the expansion is renormalised to remove ultraviolet divergences.
