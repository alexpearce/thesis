...
`signal' category.
As not all signal decays that are truly reconstructed are counted as such when 
only the `signal' category is considered, associated efficiencies are 
underestimated.
...
measured with a maximum likelihood fit to mass or \deltam\ distribution of the 
...
Hits in the \ttracker\ are not required to form long tracks, though the any 
matching hits that are found added to the track in order to improve the 
momentum resolution.
...
This \namecref{chap:prod:effs:pid} shall describe how \effpid\ is computed 
...
number of signal tracks after the \ac{PID} requirement $C_{i}'$ is computed, 
...
uncertainty on the per-decay \ac{PID} efficiency.
