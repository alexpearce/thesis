...
size, an \emph{extended} maximum likelihood likelihood fit is performed that 
relaxes normalisation requirement on the \acp{PDF} to allow for Poisson 
fluctuations around the observed number of events $N_{\text{obvs.}}$
\begin{equation}
  ...
  ,
\end{equation}
where $N_{s}$ is the \emph{yield} associated to each component model 
$f_{s}$~\cite{Barlow:1990vc} and $\vec{N}$ is the vector of those yields.
...
\section{\texorpdfstring{\lnipchisq}{log(IP chi-squared)}\ fit}
...
created from the \lnipchisq\ distribution in the lower and upper sidebands of 
the reconstructed charm hadron mass.
...
\begin{equation}
  f(\lnipchisq) = \frac{1}{\sum_{\textnormal{s}} N_{\textnormal{s}}}
                  ...
                  f_{\text{s}}(\lnipchisq).
\end{equation}
The constraint on the background yield is applied by multiplying the likelihood 
by a normal distribution whose mean is the number of background candidates 
$N_{\text{Bkg}}$ measured by the mass fit and whose width is the uncertainty on 
$N_{\text{Bkg}}$ returned by the fitter.
...
fixed to values obtained from `prefits' to the samples of prompt and secondary 
signal \ac{MC} described in \cref{chap:prod:data:mc}.
...
\ac{NLL} minimisation converged to a sensible parameter value set.
...
% TODO the rounding is OK but uncertainties are always rounded up. I guess
% that's OK?
