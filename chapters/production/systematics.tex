...
hadrons.
Uncertainties that are fully correlated, with a correlation correlation $\rho$ 
equal to 1, cancel in ratios.
In addition, testing goodness-of-fits to the cross-section measurements, as 
done by theorists, should account for correlations to be meaningful.
In what follows, the range of correlations of each uncertainty between bins and 
hadrons will be given, and a summary presented in 
\cref{chap:prod:syst:correlations}.

The systematic uncertainties are treated as nuisance parameters of the 
cross-section measurement, being propagated as an additive nuisance parameter 
on the respective term they relate to.
...
width corresponding to the size of the systematic variation being assessed.
...
cross-section measurement, are given in Tables~\ref{tab:syst:mc:stat:DzToKpi},
...
As the selection efficiency does not include the effects of the \ac{PID} 
selection, the data distributions are corrected for the \ac{PID} efficiency by 
...
(with the exception of the \ac{PID} requirements which are treated separately).
Comparisons of an example set of variables used in the \DzToKpi\ and 
\DpToKpipi\ selections are shown in 
\cref{fig:prod:syst:mc:D0ToKpi,fig:prod:syst:mc:DpToKpipi}.
...
% TODO I don't understand this. I thought it was differential in pT-y but know 
% I don't think it is. needs clarification.
...
\cref{tab:prod:syst:mc:modelling}.
...
A common set of efficiency histograms is generated and is used for computing 
the uncertainty in each bin for all charm mesons, allowing the correlations to 
be determined.
...
The relative uncertainty $\unc_{C}{\effpid}/\effpid$ is given in \cref{} for 
...
calibration sample size was larger, \acp{PDF} in track \ptot\ and \Eta\ are 
made using \acp{KDE}~\cite{Poluektov:2014rxa}.
The event multiplicity is not modelled as the \ac{PID} efficiency variation in 
this feature is seen to be smaller than that in track \ptot\ and \Eta.
...
The \acp{KDE} can be used a generative \acp{PDF}, to produce 
pseudo-experimental data with which to perform the \ac{PID} calibration.
...
\cref{tab:syst:pid:binning:DzToKpi} for \DzToKpi.
...
measurements of the visible \pp\ cross-section over time.
...
track \ptot\ and \Eta, and shown in \cref{}.
They are propagated to the cross-section measurements in an identical manner to 
that used to propagate the statistical uncertainty on the \ac{PID} efficiency, 
also arising from the finite calibration sample size, as presented in 
\cref{chap:prod:syst:pid:stat}.
...
hadrons; and due to the weighting procedure used to equalise the event 
multiplicity between the real data and the \ac{MC}.
...
\cref{tab:prod:syst:tracking:DzToKpi} for \DzToKpi.
...
\chisq\ functional form to have a parameterised asymmetry in the 
tails~\cite{LHCb-CONF-2016-010}.
Alternative models for secondary charm are the \ac{PDF} used for the prompt 
signal, and the asymmetric \chisq\ function used as an alternative prompt 
signal model.

The resulting relative systematic uncertainties are given in 
\cref{tab:syst:fitting:DzToKpi} for \DzToKpi.
...
\cref{tab:syst:signal_window:DzToKpi}, for \DzToKpi, relative to the 
...
A summary of the uncertainties entering the cross-section measurements is given 
in \cref{tab:prod:syst:summary}.
...
\begin{table}
  ...
  \label{tab:prod:syst:tracking:DzToKpi}
  \centering
...
\begin{table}
  ...
  \label{tab:syst:signal_window:DzToKpi}
  \centering
